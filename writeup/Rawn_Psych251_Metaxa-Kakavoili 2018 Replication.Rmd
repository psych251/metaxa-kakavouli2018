...
library(janitor)
####Import data
data = read.csv("data/PilotBData.csv")
turk = read.csv("data/pilotBTurk.csv")
...
#data_filtered = data_filtered[-c(2,3),]
...
ab_male_1 = data_summary %>%
  filter(Gender == "1", Category == "AB",Treatment == 1) 
ab_male_2 = data_summary %>%
  filter(Gender == "1", Category == "AB",Treatment == 2) 
ab_female_1 = data_summary %>%
  filter(Gender == "2", Category == "AB",Treatment == 1) 
ab_female_2 = data_summary %>%
  filter(Gender == "2", Category == "AB",Treatment == 2) 

#t.test(ab_male_1["MeanQuestion"],ab_male_2["MeanQuestion"])
g <- ggplot(ab, aes(y=TotalMeanQuestion,x=Gender, fill=Treatment)) + 
  geom_bar(position="dodge", stat="identity") +
  scale_fill_brewer(palette="Set1") +
  ggtitle("Ambient Belonging Proof of Concept Results") +
  ylab("Mean Question Response")

```
```{r}
p <- ggplot(data_stats, aes(y=MeanQuestion,x=Gender,fill=Treatment)) + 
  geom_bar(position="dodge", stat="identity") +
  scale_fill_brewer(palette="Set1") +
  ggtitle("Ambient Belonging Proof of Concept Results") +
  ylab("Mean Question Response") +
